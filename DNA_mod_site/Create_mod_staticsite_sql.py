...
def render_image(smiles, name):
    pwd = FILE_PATH + '/DNA_mod_site/static/images'
...
def get_modifications():
    ...
    ans = subprocess.call("dbrow '_base_id =~ /c/' -i mod_base.fsdb | dbjoin -i - -i base.fsdb -R base_id | dbjoin -i - -i name.fsdb -R name_id | dbjoin -i - -i cov_modification.fsdb -R cmod_id | dbjoin -i - -i base_properties.fsdb -R property_id > result2.fsdb",cwd = FILE_PATH, shell = True)
...
def create_html_pages():
    ...
    for BASE in BASES:
        links = []
        with open(FILE_PATH_UP + '/result.fsdb', 'r') as f1:
            ...
            for line in reader:
                ...
                if data[5] == BASE[0].lower():
                    ...
                    writefile = FILE_PATH + '/static/'+ data[10] +'.html'
                    f = open(writefile, 'w+')
